...
function createCalendar() {
    ...
    if (!name || window.calendars[name]) {
        alert('Please enter a unique non-empty name for the calendar.');
        return;
    }
    ...
    monthNames.forEach((month, index) => {
        ...
        // Determine the start day of the month
        const firstDay = new Date(year, index, 1).getDay();
        
        // Create leading empty days
    ...
    if (data) {
        Object.keys(data).forEach(name => {
            ...
            data[name].forEach((filled, index) => {
                if (filled) {
                    days[index].classList.add('filled');
                }
            });
